...
function FIFO() {
	...
	timer = setInterval(function() {
		if(i < length) {
			flog = 0;
			i++;
			stackStatus.html(length - i );
			loseStatus.html(parseInt(((i - (flogTime)) /(i))* 100) + '%');
			...
			if(flog != 1) {
				var div = $('<div/>').addClass('box yellow').text(_stack[i-1]).attr('style', 'display:inline-block');
...
function LRU() {
	...
	timer = setInterval(function() {
		if(i <= length) {
			...
			loseStatus.html(parseInt(((i - flogTime + 1) / (i+1))* 100) + '%');
			if((length - i - 1) == 0) 
				$('.manage.stack').attr('class', 'stack green manage');
			var box = $('#boxs .box');
			for(var k = 0; k < box.length; k++) {
